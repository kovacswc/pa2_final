...
class Mininet( object ):
    ...
    def iperf_one_par( self, hosts=None, l4Type='TCP', udpBw='10M', fmt=None,
                       seconds=5, port=5001,num_conn='', par = ''):
        client, server = hosts
        ...
        fh = open(full_fn, "w")
        client.popen(iperfArgs + '-t %d -c ' % seconds +
                     server.IP()  + ' &',
                     stdout = fh)

    def iperf_set_servers(self, hosts = None, seconds = 5, port = 5001,
                          num_conn = '', par = ''):
        client, server = hosts
        client, server = hosts
        fn = client.name[1:] + '_'+ num_conn  + '.txt'
    ...
    def iperf_run_clients(self, hosts = None, seconds = 5, port = 5001,
                          num_conn = '', par = ''):
        
        client, server = hosts
        fn = client.name[1:] + '_'+ num_conn  + '.txt'
        output('Testing client %s using port %s\n' % ( client.name, port))
        iperfArgs = 'iperf -p %d ' % port
        iperfArgs += par
        # if not waitListening( client, server.IP(), port ):
        #     print "Could not connect to iperf on the port"
    ...
    def iperf_par(self, defSec = 5):
        print "working indirection"
        for h in self.hosts:
            h.cmd( 'killall -9 iperf' )

        hostRecv = range(len(self.hosts))
        hostPairs = []
        for hostSend in range(len(self.hosts)):
            recv = random.choice(hostRecv)
            while recv == hostSend:
                recv = random.choice(hostRecv)
            hostPairs.append((hostSend,recv))
            hostRecv.remove(recv)
        startPort = 5001
        # for (i,j) in hostPairs:
            
        #     self.iperf_one_par((self.hosts[i], self.hosts[j]), port = startPort, num_conn="one", seconds = defSec)
        #     startPort += 1
            
        for (i,j) in hostPairs:
            self.iperf_set_servers((self.hosts[i], self.hosts[j]), port = startPort,num_conn="one", seconds = defSec)
            startPort += 1

        startPort = 5001
        for (i,j) in hostPairs:
            self.iperf_run_clients((self.hosts[i], self.hosts[j]), port = startPort,num_conn="one", seconds = defSec)
            startPort += 1

    def iperf_par8(self, defSec = 5):
        for h in self.hosts:
            h.cmd( 'killall -9 iperf' )
        ...
        #     #for k in range(8):
        #     #num_conn=str(k)
        #     self.iperf_one_par((self.hosts[i], self.hosts[j]), port = startPort,par = "-P 8 ", num_conn="eight", seconds = defSec)
        #     startPort += 8

        for (i,j) in hostPairs:
            self.iperf_set_servers((self.hosts[i], self.hosts[j]), port = startPort,num_conn="eight",par = "-P 8 ",  seconds = defSec)
            startPort += 8
        sleep(1)
        startPort = 5001
        for (i,j) in hostPairs:
            self.iperf_run_clients((self.hosts[i], self.hosts[j]), port = startPort,num_conn="eight",par = "-P 8 ", seconds = defSec)
            startPort += 8
